<div class="relative bg-white dark:bg-gray-800 dark:text-gray-200">
    <div class="max-w-7xl mx-auto px-4 sm:px-6">
        <div class="flex justify-between items-center border-b-2 border-gray-500 py-6 md:justify-start md:space-x-10">
        <div class="flex justify-start lg:w-0 lg:flex-1">
            <a href="{{ route('dashboard') }}" class="text-base text-gray-500 dark:text-gray-300 font-semibold text-xl leading-tight">
                @yield('title')
            </a>
        </div>
        ...
        <div class="hidden md:flex items-center justify-end md:flex-1 lg:w-0">
            <div class="flex justify-center items-center">
                <div class="relative rounded-full w-12 h-6 transition duration-200 ease-linear"
                    :class="[darkMode === true ? 'bg-gray-700' : 'bg-orange-300']">
                    <label for="toggle"
                            class="absolute left-0 bg-white border-2 mb-2 w-6 h-6 rounded-full transition transform duration-100 ease-linear cursor-pointer"
                            :class="[ darkMode === true ? 'translate-x-full border-gray-400' : 'translate-x-0 border-orange-500']">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-orange-600" viewBox="0 0 20 20" fill="currentColor" x-show="!darkMode">
